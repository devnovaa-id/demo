class RandomQuotesApp {
  ...
  initElements() {
    ...
    // Ensure some elements exist to avoid runtime errors in case DOM missing
    if (this.quoteCounterEl) this.quoteCounterEl.textContent = this.quoteCounter;
  }

  setupEventListeners() {
    // Quote Actions
    if (this.newQuoteBtn) this.newQuoteBtn.addEventListener('click', () => this.getRandomQuote());
    if (this.copyQuoteBtn) this.copyQuoteBtn.addEventListener('click', () => this.copyToClipboard());
    if (this.tweetQuoteBtn) this.tweetQuoteBtn.addEventListener('click', () => this.shareOnTwitter());
    if (this.downloadQuoteBtn) this.downloadQuoteBtn.addEventListener('click', () => this.generateCanvasImage());

    // Scraping Controls
    if (this.scrapeNowBtn) this.scrapeNowBtn.addEventListener('click', () => this.scrapeQuotes());
    if (this.clearCacheBtn) this.clearCacheBtn.addEventListener('click', () => this.clearCache());
    if (this.testApiBtn) this.testApiBtn.addEventListener('click', () => this.testApiConnection());

    // Canvas Actions
    if (this.saveImageBtn) this.saveImageBtn.addEventListener('click', () => this.saveCanvasImage());
    if (this.copyImageBtn) this.copyImageBtn.addEventListener('click', () => this.copyCanvasImage());
    if (this.shareImageBtn) this.shareImageBtn.addEventListener('click', () => this.shareCanvasImage());
    if (this.closeCanvasBtn) this.closeCanvasBtn.addEventListener('click', () => this.hideCanvasPreview());

    // Settings
    if (this.waitTimeSlider && this.waitTimeValue) {
  ...
  async scrapeQuotes() {
    ...
    const page = this.pageSelect && this.pageSelect.value === 'random'
      ? Math.floor(Math.random() * 10) + 1
      : parseInt(this.pageSelect ? this.pageSelect.value : 1);

    const waitTime = parseFloat(this.waitTimeSlider ? this.waitTimeSlider.value : 2) * 1000;
  ...
  displayQuote(quote) {
    ...
    if (this.quoteTags) {
      ...
      if (this.includeTags && this.includeTags.checked && quote.tags.length > 0) {
      ...
      } else if (this.includeTags && this.includeTags.checked) {
  ...
  renderHistory() {
    if (!this.quotesHistory) return;
    this.quotesHistory.innerHTML = '';

    if (this.history.length === 0) {
      this.quotesHistory.innerHTML =
        '<div class="empty-history"><i class="fas fa-inbox"></i><p>No history yet. Start scraping quotes!</p></div>';
      return;
    }
  ...
  async generateCanvasImage() {
    ...
    const originalText = this.downloadQuoteBtn ? this.downloadQuoteBtn.innerHTML : '';
    if (this.downloadQuoteBtn) {
      this.downloadQuoteBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
      this.downloadQuoteBtn.disabled = true;
    }

    try {
      if (!this.ctx) throw new Error('Canvas not available');
      ...
      // Get quote data
      const text = this.quoteText ? this.quoteText.textContent.replace(/^"|"$/g, '') : '';
      const author = this.quoteAuthor ? this.quoteAuthor.textContent : '';
  ...
  hideCanvasPreview() {
    if (this.canvasPreview) this.canvasPreview.classList.add('hidden');
  }

  async copyToClipboard() {
    const quote = this.quoteText ? this.quoteText.textContent : '';
    const author = this.quoteAuthor ? this.quoteAuthor.textContent : '';
  ...
  shareOnTwitter() {
    const quote = (this.quoteText ? this.quoteText.textContent.replace(/^"|"$/g, '') : '').trim();
    const author = this.quoteAuthor ? this.quoteAuthor.textContent : '';
  ...
  clearCache() {
    ...
    this.history = [];
    this.quoteCounter = 1;
    if (this.quoteCounterEl) this.quoteCounterEl.textContent = '1';
  ...
  updateStats() {
    if (this.totalQuotesEl) this.totalQuotesEl.textContent = this.currentQuotes.length;
    if (this.lastUpdatedEl) this.lastUpdatedEl.textContent = new Date().toLocaleTimeString([], {
      hour: '2-digit',
      minute: '2-digit'
    });
  }
  ...
  showLoading() {
    ...
    if (this.quoteContainer) this.quoteContainer.classList.add('hidden');
    if (this.canvasPreview) this.canvasPreview.classList.add('hidden');
  }

  hideLoading() {
    if (this.loadingEl) this.loadingEl.classList.add('hidden');
    if (this.quoteContainer) this.quoteContainer.classList.remove('hidden');
  }
  ...
  showToast(message, type = 'info') {
    ...
    if (this.toastIcon) this.toastIcon.className = `fas ${icons[type]}`;
    if (this.toast) this.toast.style.borderLeftColor = colors[type];
    if (this.toastMessage) this.toastMessage.textContent = message;

    // Show toast
    if (this.toast) this.toast.classList.add('show');
